...
@task
def read_alpha_vantage_data(file_pattern: str = "db/fundamentals_*.parquet") -> Optional[pd.DataFrame]:
    """
    ...
    logger.info(f"Reading Alpha Vantage data from {file_pattern}")
    ...
    try:
        ...
        if not files:
            logger.warning(f"No Alpha Vantage files found matching {file_pattern}")
            return None
        ...
        logger.info(f"Loaded {len(df)} rows from Alpha Vantage data")
        
        return df
        
    except Exception as e:
        logger.error(f"Error reading Alpha Vantage data: {e}")
        return None


@task
def read_xbrl_data(file_pattern: str = "db/xbrl_data_*.parquet") -> Optional[pd.DataFrame]:
    """
    ...
    """
    logger = get_run_logger()
    logger.info(f"Reading XBRL data from {file_pattern}")
    ...
    try:
        ...
        if not files:
            logger.warning(f"No XBRL files found matching {file_pattern}")
            return None
        ...
        df = pd.read_parquet(latest_file)
        logger.info(f"Loaded {len(df)} rows from XBRL data")
        
        return df
        
    except Exception as e:
        logger.error(f"Error reading XBRL data: {e}")
        return None
...
def merge_fundamental_data(
    ...
    """
    logger = get_run_logger()
    logger.info("Merging Alpha Vantage and XBRL data")
    
    try:
        if alpha_vantage_df is None and xbrl_df is None:
            logger.warning("Both DataFrames are None, nothing to merge")
            return None
        
        if alpha_vantage_df is None:
            logger.info("Only XBRL data available")
            return xbrl_df
        
        if xbrl_df is None:
            logger.info("Only Alpha Vantage data available")
            return alpha_vantage_df
        
        # Merge on ticker (case-insensitive)
        alpha_vantage_df["ticker_lower"] = alpha_vantage_df.get("ticker", "").str.lower()
        ...
        if "ticker_alpha_vantage" in merged.columns and "ticker_xbrl" in merged.columns:
            merged["ticker"] = merged["ticker_alpha_vantage"].fillna(merged["ticker_xbrl"])
            merged = merged.drop(["ticker_alpha_vantage", "ticker_xbrl"], axis=1)
        
        logger.info(f"Merged data contains {len(merged)} unique tickers")
...
@task
def enrich_with_calculated_metrics(merged_df: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
    """
    ...
    """
    logger = get_run_logger()
    logger.info("Enriching data with calculated metrics")
    ...
    try:
        ...
        # PEG Ratio (P/E divided by growth rate) - placeholder
        ...
        ratio_columns = [col for col in df.columns if "ratio" in col.lower() and "pe" not in col.lower()]
        if ratio_columns:
            for col in ratio_columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")
            df["health_score"] = df[ratio_columns].mean(axis=1)
        
        # Value Score (inverse of P/E and P/B ratios)
        if "pe_ratio_numeric" in df.columns:
            df["value_score"] = 1 / df["pe_ratio_numeric"]
        
        df["enriched_at"] = datetime.now().isoformat()
        
        logger.info(f"Added {len([col for col in df.columns if col not in merged_df.columns])} calculated metrics")
...
def save_merged_data_to_parquet(
    ...
    try:
        ...
        file_path = os.path.join(output_dir, f"{filename_prefix}_{timestamp}.parquet")
...
def get_ticker_analysis(merged_df: pd.DataFrame, ticker: str) -> dict:
    """
...
def compare_tickers(merged_df: pd.DataFrame, tickers: list[str], metric: str) -> Optional[pd.DataFrame]:
    """
    ...
    comparison_df = merged_df[merged_df.get("ticker", "").str.upper().isin(tickers_upper)]
